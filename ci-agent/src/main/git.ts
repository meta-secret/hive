...
export class CiRepository {
  ...
  > {
    ...
    for (const [key, value] of [
      ...
      const configured = await this.trustedGit({
  ...
  private async pushAuthenticatedBranch(): Promise<void> {
    ...
    const authEnv = {
      ...
      GIT_CONFIG_VALUE_0: `AUTHORIZATION: basic ${Buffer.from(`x-access-token:${token.value}`).toString("base64")}`,
    };
    await execFileAsync(
      "git",
      this.trustedGitArgs({ args: ["push", "-u", "origin", "HEAD"] }),
      {
        env: authEnv,
      },
    );
...
const REPOSITORY_GENERATED_PATHS = new Set([
  "/console/src/generated/observer-contract.ts",
]);
