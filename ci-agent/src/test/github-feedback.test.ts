...
void test("exact-head iteration markers require a trusted exact request", () => {
  const marker = "<!-- hive-codex-review:head-sha -->";
  assert.equal(
...
void test("only a trusted canonical request marker is repository status", () => {
  ...
  assert.equal(
    new GitHubIsRepositoryStatusComment({
      authorAssociation: "OWNER",
      body: `@codex review\n\n${marker}`,
      cursorMarker: "<!-- hive-cursor-review:head-sha -->",
      marker,
      user: { login: "cypherkitty" },
    }).execute(),
    true,
...
void test("provider status text is authenticated before exclusion", () => {
  ...
  const base = {
    ...
    body: status,
    cursorMarker: "<!-- hive-cursor-review:head-sha -->",
    marker: "<!-- hive-codex-review:head-sha -->",
  };
  assert.equal(
    new GitHubIsRepositoryStatusComment({
      ...base,
...
void test("ordinary workflow comments remain substantive feedback", () => {
  const base = {
    authorAssociation: "NONE",
    cursorMarker: "<!-- hive-cursor-review:head-sha -->",
    marker: "<!-- hive-codex-review:head-sha -->",
  };
  for (const body of ["### Preview deployed", "Automated verification note"]) {
    assert.equal(
      new GitHubIsRepositoryStatusComment({
        ...base,
        body,
