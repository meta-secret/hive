...
import { GitHubTokenEnvironment } from "./github-token.js";
export class CiFixCommand {
  ...
  async runCiFix(): Promise<Result<CiFixOutcome, CiFailure>> {
    ...
    const client = new GitHubEnvironment(this.environment).createOctokit();
    ...
    if (openPr.value.kind === OpenPrLookupKind.Found) {
      ...
      const loaded = new CiAgentEnvironment(this.environment).loadConfig();
      ...
      const pushed = await new CiRepository(
        repoRoot,
        this.environment,
      ).pushFixBranch({
        fixBranch,
    ...
    log.info(
      `PR #${prNumber} exact head ${published.value.headSha} verified and handed to the continuing delivery owner`,
    );
    log.info(
  ...
  private async auditExistingDependencyBranch(
    ...
    const token = new GitHubTokenEnvironment(this.environment).resolve();
    ...
    this.environment.GIT_CONFIG_VALUE_0 = `AUTHORIZATION: basic ${Buffer.from(`x-access-token:${token.value}`).toString("base64")}`;
