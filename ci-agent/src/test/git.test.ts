...
void describe("countAuthoredNumstat", () => {
  ...
  void it("reports generated, lock, snapshot, vendor, binary, and pure rename rows separately", () => {
    const numstat = [
      ...
      "14\t2\tconsole/src/generated/observer-contract.ts",
      "-\t-\tassets/demo.png",
      "0\t0\t",
      "src/old.ts",
      "src/new.ts",
...
void describe("implementation working tree", () => {
  void it("trusts only the exact worktree without persistent global configuration", async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), "hive-ci-agent-safe-"));
    ...
    try {
      ...
      await assert.rejects(access(globalConfig), /ENOENT/);
      const { stdout: userName } = await execFileAsync("git", [
        "-C",
        repoRoot,
        "config",
        "--local",
      ...
      assert.equal(userName.trim(), "github-actions[bot]");
      assert.deepEqual(
        new CiRepository(repoRoot).trustedGitArgs({ args: ["status"] }).slice(
          0,
          4,
        ),
        ["-C", repoRoot, "-c", `safe.directory=${repoRoot}`],
      );
      assert.equal(
        await new CiRepository(repoRoot)
  ...
  void it("disables editor-controlled hooks for trusted publication", async () => {
    const tempRoot = await mkdtemp(join(tmpdir(), "hive-ci-agent-hooks-"));
    const repoRoot = join(tempRoot, "repo");
    ...
    const previousHiveToken = process.env.HIVE_GITHUB_PAT;
    ...
    try {
      ...
      delete process.env.HIVE_GITHUB_PAT;
      process.env.GITHUB_TOKEN = "publication-secret";
      ...
      if (typeof previousHiveToken === "string")
        process.env.HIVE_GITHUB_PAT = previousHiveToken;
      else delete process.env.HIVE_GITHUB_PAT;
      await rm(tempRoot, { recursive: true, force: true });
    }
  });

  void it("excludes forced runtime artifacts while retaining authored changes", async () => {
    const repoRoot = await mkdtemp(join(tmpdir(), "hive-ci-agent-git-"));
    try {
      await execFileAsync("git", ["-C", repoRoot, "init"]);
      await writeFile(join(repoRoot, "README.md"), "base\n");
      await execFileAsync("git", ["-C", repoRoot, "add", "README.md"]);
      ...
      await writeFile(
        join(repoRoot, ".hive-workbench-plan.md"),
        "trusted plan\n",
      );
      await writeFile(
        join(repoRoot, ".hive-workbench-worklog.md"),
        "candidate log\n",
      );
      await execFileAsync("git", [
        "-C",
        ...
        "add",
        "-f",
        ".hive-workbench-plan.md",
        ".hive-workbench-worklog.md",
      ]);
      assert.equal(
        await new CiRepository(repoRoot)
          .hasWorkingTreeChanges()
