...
void test("validation isolates secrets, preserves wrapper vars, and denies network overrides", async () => {
  const root = await mkdtemp(join(tmpdir(), "hive-validation-test-"));
  const bin = join(root, "bin");
  const log = join(root, "docker.log");
  const trustedHome = join(root, "home");
  const builder = "hive-builder";
  await mkdir(bin);
  ...
  const hostEnvironment: NodeJS.ProcessEnv = {
    ...
    HIVE_GITHUB_PAT: "github",
    SCCACHE_S3_ACCESS_KEY_FILE: "/trusted/sccache-access",
    HIVE_BUILDKIT_REMOTE: "1",
    HIVE_PR_BUILDX_BUILDER: builder,
  };
  ...
  try {
    await new DependencyFixWithValidationEnvironment({
      ...
      operation: async (environment) => {
        ...
        assert.equal(environment.HIVE_BUILDKIT_REMOTE, "1");
        ...
        await new DependencyFixRepository("/repo")
          .runRustDependencyUpdateValidation({
            ...
            runner: async (_command, args, { env }) => {
              ...
              assert.equal(
                env.HIVE_VALIDATION_DOCKER,
                environment.HIVE_VALIDATION_DOCKER,
              );
...
void test("dependency update scope accepts only regular Rust mission files", async () => {
  const root = await mkdtemp(join(tmpdir(), "hive-fix-scope-"));
  try {
    const allowed = [
      "Cargo.toml",
      "worker/src/lib.rs",
      "worker/tests/neo4j_store.rs",
    ];
    ...
    for (const [path, message] of [
      ...
      ["Taskfile.yml", /orchestration control/],
      ["worker/build.rs", /orchestration control/],
    ] as const)
      await rejects(path, " M", message);
    const pinned =
      'hickory = { git = "https://github.com/meta-secret/hickory-dns.git" }\n';
    await writeFile(join(root, "Cargo.toml"), pinned);
    await new DependencyFixRepository(root)
      .assertRustDependencyUpdateChangeSet({
        changes: [{ path: "Cargo.toml", status: " M" }],
    ...
    await rejects("Cargo.toml", " M", /non-crates.io/);
    await writeFile(
    ...
    const symlinkPath = "worker/src/linked.rs";
