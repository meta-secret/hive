...
const RUST_DEPENDENCY_FILES = new Set(["Cargo.lock", "Cargo.toml"]);
const RUST_DEPENDENCY_ROOTS = ["worker/", "vendor/"] as const;
...
export class RustDependencyPath {
  ...
  isAllowed(): boolean {
    ...
    if (
      !RUST_DEPENDENCY_FILES.has(path) &&
      !RUST_DEPENDENCY_ROOTS.some((root) => path.startsWith(root))
    ) {
      return false;
    }
