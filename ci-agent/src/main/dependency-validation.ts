...
export class DependencyFixWithValidationEnvironment<T> {
  ...
  async execute(): Promise<Result<T, CiFailure>> {
    ...
    const root = await ResultAsync.fromPromise(
      mkdtemp(join(tmpdir(), "hive-validation-")),
      (): CiFailure => ({
  ...
  private async prepare(
    ...
    const builder = base.HIVE_PR_BUILDX_BUILDER;
    ...
    return ok({
      ...
      DOCKER: docker,
      HOME: home,
      HIVE_VALIDATION_DOCKER: dockerHost,
      PATH: `${bin}:${base.PATH || ""}`,
      SCCACHE_OPTIONAL: "1",
...
const VALIDATION_ENV_ALLOWLIST = new Set([
  ...
  "LC_ALL",
  "NO_COLOR",
  "HIVE_BUILDKIT_REMOTE",
  "HIVE_PR_BUILDX_BUILDER",
  "PATH",
  "RUNNER_TOOL_CACHE",
  "SHELL",
  "TERM",
...
case "\${1:-}" in
  ...
  buildx)
    ...
    case "$sub" in
      ...
      create)
        [ "$*" = "--name \${HIVE_PR_BUILDX_BUILDER:-} --driver docker-container --bootstrap" ] || exit 97
        exec "$real" buildx create "$@" ;;
      inspect|use|version) exec "$real" buildx "$sub" "$@" ;;
      rm)
        [ "$*" = "--force \${HIVE_PR_BUILDX_BUILDER:-}" ] || exit 97
        exec "$real" buildx rm "$@" ;;
